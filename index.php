...
$db=new dbconnection;
   $select_cart = mysqli_query($db->conn, "SELECT * FROM `cart` WHERE name = '$product_name' AND user_id = '$user_id'") or die('query failed');

   if(mysqli_num_rows($select_cart) > 0){
      $message[] = 'المنتج أضيف بالفعل إلى عربة التسوق!';
   }else{
      mysqli_query($db->conn, "INSERT INTO `cart`(user_id, name, price, image, quantity) VALUES('$user_id', '$product_name', '$product_price', '$product_image', '$product_quantity')") or die('query failed');
...
if(isset($_POST['update_cart'])){
   ...
   mysqli_query($db->conn, "UPDATE `cart` SET quantity = '$update_quantity' WHERE id = '$update_id'") or die('query failed');
   $message[] = 'تم تحديث كمية سلة التسوق بنجاح!';
}

if(isset($_GET['remove'])){
   $remove_id = $_GET['remove'];
   mysqli_query($db->conn, "DELETE FROM `cart` WHERE id = '$remove_id'") or die('query failed');
...
<div class="user-profile">
   ...
   include_once 'C:\wamp64\www\Student Services\config\dbconnection.php';
   $db=new dbconnection;
      $select_user = mysqli_query($db->conn, "SELECT * FROM `users` WHERE id = '$user_id'") or die('query failed');
...
<div class="products">
   ...
   include_once 'C:\wamp64\www\student services\config\dbconnection.php';
   $db=new dbconnection;
   $result = mysqli_query($db->conn, "SELECT * FROM products");      
...
<div class="shopping-cart">
   ...
   <table>
      ...
      <tbody>
      <?php
         $cart_query = mysqli_query($db->conn, "SELECT * FROM `cart` WHERE user_id = '$user_id'") or die('query failed');
         $grand_total = 0;
